...
document.addEventListener('DOMContentLoaded', () => {
    // Восстановлено: Базовый звук удара (чтобы игра не ломалась при кликах)
    const hitSoundEffect = new Audio('assets/sounds/hitsound.mp3'); 
    hitSoundEffect.volume = 0.7;

    // Ваши настройки: Инициализация звука идеального прохождения
    const perfectSoundEffect = new Audio('assets/sounds/sss-dmc-v.mp3');
    perfectSoundEffect.volume = 0.5;
    let isPerfectRun = true; 

    // Ваши настройки: Инициализация звука при проигрыше
    const loseSoundEffect = new Audio('assets/sounds/cat-laugh-meme-1.mp3');
    loseSoundEffect.volume = 0.3;

    // Массив для хранения летящих текстов очков
    let floatingTexts = [];

    // Переменная для управления прозрачностью бокового свечения
    let sideGlowAlpha = 0;
